...
checkinstall --default --pkgname=libopencv-dev --pkgversion=$ACT_VER --pkgrelease=$ACT_REL --pkgsource="https://github.com/opencv/opencv/commit/$commit" --maintainer="OpenCV" --nodoc --requires="libc6 \(\>= 2.14\),libgcc1 \(\>= 1:3.0\),pkg-config" \
